...
from nlp.common.errors import ConfigurationError
...
def import_all_modules_for_register(custom_module_paths=None):
    """
    sub module在被import前不会被注册，这里需要维护一个module/path的映射表
    Args:
        custom_module_paths:
    Returns:
    """
    modules_to_import = []
    from nlp.common.ALL_MODULES import ALL_MODULES
    for base, modules in ALL_MODULES:
        for name in modules:
            full_name = base + "." + name
            modules_to_import.append(full_name)
    if isinstance(custom_module_paths, list):
        modules_to_import += custom_module_paths
